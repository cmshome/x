...
上图所示的 Eden 区、两个 Survivor 区都属于新生代（为了区分，这两个 Survivor 区域按照顺序被命名为 s0 和 s1），中间一层属于老年代。  
...
# 三 HotSpot 虚拟机对象探秘
下面来详细的了解一下 HotSpot 虚拟机在 Java 堆中对象分配、布局和访问的全过程。
## 3.1 对象的创建
...
|--    |程序计数器         |虚拟机栈|本地方法栈|堆     |方法区（永久代）|类压缩空间|本机内存 元空间|
|:-----|:---              |:----- |:-----   |:----- |:-----         |:-----   |:-----|
|英文名|program counter register|JVM stacks|native method stacks|java heap|method area|Compressed class space|Metaspace|
|线程私有|是              |是    |   是      | 否     |   否          |   否    |  否 |
|是否健在|      是   | 是         |  是       |  是     |否（自1.8始终无了）|   是      | 是     |
|什么异常|无        |StackOverflow & OOM|StackOverflow & OOM|OOM |  OOM       |    -  |-         |
|作用|控制指令的各种跳转|Java方法的执行的内存模型|Native方法的执行的内存模型|存放对象实例|  存储已被虚拟机加载的类信息、常量、静态变量、即时编译器编译后的代码等数据| 存类的字节码 | 用来存放 class metadata 元数据 |
|主要存啥|当前线程所执行的字节码的行号指示器|局部变量表、操作栈、动态链接、方法出口等信息| 局部变量表、操作栈、动态链接、方法出口等信息| 存放对象实例| 存储已被虚拟机加载的类信息、常量、静态变量、即时编译器编译后的代码等数据|         |      |
|比如说有啥| 正在执行的虚拟机字节码指令的地址 |局部变量 |  局部变量表| 实例对象  |   类信息、常量、静态变量 |class|   常量池 ，方法的字节码、局部变量表、异常表、参数信息等  |
|大小取决于| - |局部变量表 |  局部变量表| 实例对象总数和大小  | 类的总数大小，常量池的大小，方法的大小 |类的总数大小|   类的总数大小，常量池的大小，方法的大小  |

- [link 2 github--md源文件链接][0]  
- [link 2 csdn][1]
...
[4]: https://img-blog.csdnimg.cn/20210421181804833.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzI3MDkzNDY1,size_16,color_FFFFFF,t_70
[5]: https://img-blog.csdnimg.cn/20210422094649312.png
[6]: https://img-blog.csdnimg.cn/20210422094905448.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzI3MDkzNDY1,size_16,color_FFFFFF,t_70
[7]: https://img-blog.csdnimg.cn/20210422100337437.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzI3MDkzNDY1,size_16,color_FFFFFF,t_70
[8]: https://img-blog.csdnimg.cn/20210422100251402.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzI3MDkzNDY1,size_16,color_FFFFFF,t_70
